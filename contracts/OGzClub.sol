...
contract OGzClub is ERC20, Ownable {
    ...
    /**
     * @dev As there is a total of 62 data in the previous contract, the data to be transferred to the new contract is limited to 62.
     */
    uint32 private constant MAX_INITIALIZE_REFERRAL_DATA_COUNT = 62;
     ...
     */
    function initializeReferralDatas(
        Referrals[] memory referrals
    ) external onlyOwner returns (bool) {
        require(
            totalInitializated + referrals.length <=
                MAX_INITIALIZE_REFERRAL_DATA_COUNT,
            "Initialization is done"
        );
        require(
            referrals.length > 0,
            "Referrals array length must be greater than zero"
        );
        totalInitializated += referrals.length;
        for (uint256 i = 0; i < referrals.length; ) {
            _preferredNicknames[referrals[i].referral] = 1;
            require(
                bytes(_referralNickname[referrals[i].referral]).length == 0,
                "You already have a nickname"
            );
            require(
                bytes(referrals[i].nickname).length != 0 &&
                    bytes(referrals[i].nickname).length <= 64,
                "Nickname must be between 1 and 64 characters"
            );
            require(
                _nickNames[referrals[i].nickname] == address(0),
            ...
            emit CreatedLink(
                ...
                referrals[i].nickname,
                referrals[i].timestamp
            );
            if (referrals[i].referees.length > 0) {
                bytes32 referrerNicknameHash = keccak256(
                    abi.encodePacked(_referralNickname[referrals[i].referral])
                );
                for (uint256 k = 0; k < referrals[i].referees.length; ) {
                    require(
                        _referrals[referrals[i].referral] !=
                            referrals[i].referees[k].referrer &&
                            _referrals[referrals[i].referees[k].referrer] ==
                            address(0) &&
                            referrerNicknameHash !=
                    ...
                    _referrals[referrals[i].referees[k].referrer] = referrals[i]
                        .referral;
                    emit RegisteredReferrence(
                        referrals[i].referral,
                        referrals[i].nickname,
                        referrals[i].referees[k].referrer,
                        referrals[i].referees[k].timestamp
                    );
                    unchecked {
                        k++;
                    }
